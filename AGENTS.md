...
3. Issue evidence의 `Execution mode`와 `Execution mode reason`을 먼저 고정하고, Issue에 직접 연결된 문서만 읽어 목표, 제외 범위, 검증 계획을 확정합니다.
4. 구현·Review·QA 작업에는 `.codex/skills/coffee-order-issue-loop/SKILL.md`를 적용합니다.

## 반드시 지킬 경계

- 각 Dev Agent와 worktree는 한 번에 Issue 하나만 처리합니다. Main Coordinator는 쓰기 범위와 도메인 계약이 겹치지 않는 독립 Issue를 최대 권장 동시 Dev 수 2 안에서 병렬 조정할 수 있습니다.
