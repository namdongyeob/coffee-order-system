...
- Redis처럼 유실 가능한 저장소는 항상 파생 데이터로 취급하고, 원천 데이터(DB)로부터 재구성할 수 있는 복구 경로를 둡니다.
...
| 스키마 관리 | Flyway versioned migration(`V1`~`V5`) | `db/migration`을 스키마 변경의 단일 경로로 사용하고 JPA는 매핑만 담당합니다. |
...
│   ├── consumer / rebuild / service
└── recovery         (DLT 재발행)
```

기능별로 패키지를 먼저 나누고, 그 안에서 Controller-Service-Repository 3계층으로 구분합니다. 계층 경계 규칙은 `docs/architecture/layered-design-policy.md`를 따릅니다.
...
- Kafka Consumer(`ranking-consumer-group`)는 `eventId` unique 이력(`processed_event`)으로 중복 이벤트를 무시합니다. 반복 재시도 후 실패한 메시지는 `order.completed.DLT`로 이동합니다.

### 인기 메뉴 랭킹

- Kafka Consumer가 성공 처리한 주문마다 Redis ZSET(`popular:menus:{yyyy-MM-dd}`)의 메뉴별 점수를 증가시킵니다. 이때 eventId 기준 Lua 원자 스크립트로 같은 이벤트가 재처리돼도 점수가 중복 증가하지 않도록 보장합니다.
- 조회 시 최근 7일 날짜별 key를 `ZUNION`으로 합산하고, 점수 내림차순·메뉴 ID 오름차순으로 정렬해 Top 3를 반환합니다.
- Redis는 원천 데이터가 아닌 파생 데이터이며, 유실 시 `ranking-rebuild-group`으로 Kafka topic을 replay해 재구성합니다.
...
전체 명세는 `docs/api/api-spec.md`를 참고합니다.
...
Kafka UI는 http://localhost:18080, RedisInsight는 http://localhost:15540 입니다. 상세 절차와 실제 확인 명령은 `docs/operations/local-runbook.md`를 참고합니다.

## 테스트와 검증

- Java 테스트 76개(단위·MVC slice·MySQL/Kafka/Redis Testcontainers 통합) 전부 통과합니다. 동시 주문 락 경합, Kafka Consumer 중복 처리, DLT 이동, Outbox 발행 실패·재시도 시나리오를 실제 인프라로 검증했습니다.
- CI(GitHub Actions `quality-gates`)가 모든 PR에서 하네스 테스트, 컴파일, 전체 Gradle 테스트, 링크 검사를 실행합니다.
- k6 부하 테스트 결과는 `docs/performance/k6-results.md`, DB 인덱스·EXPLAIN 근거는 `docs/db/indexing-explain.md`를 참고합니다.
...
- ADR(설계 결정과 대안 비교): `docs/adr/`.
- API 명세: `docs/api/api-spec.md`.
- DB/ERD: `docs/db/erd.md`.
- 테스트 전략: `docs/testing/test-strategy.md`.
- 운영 Runbook: `docs/operations/local-runbook.md`, `docs/operations/kafka-redis-runbook.md`.
