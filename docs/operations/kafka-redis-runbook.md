...
`ranking_event_ledger` cleanup은 한 scheduler tick에서 한 번의 bounded batch만 실행합니다. 안전한 기본값과 활성화 시 필수 입력은 다음과 같습니다.

| 설정 | 기본값 | 의미 |
| --- | --- | --- |
| `ranking.ledger.cleanup.enabled` | `false` | scheduler 활성화 여부 |
...
| `ranking.ledger.cleanup.kafka-retention` | 기본값 없음, 활성화 시 필수 | 운영자가 확인한 `order.completed` effective retention |
| `ranking.ledger.cleanup.dlt-retention` | 기본값 없음, 활성화 시 필수 | 운영자가 확인한 `order.completed.DLT` effective retention |
| `ranking.ledger.cleanup.maximum-rebuild-recovery-window` | 기본값 없음, 활성화 시 필수 | rebuild를 재개할 수 있는 최대 기간 |
...
Kafka와 DLT의 값은 자동 추정값이 아니라 배포 시 topic의 `retention.ms`와 broker default를 Admin API 또는 `kafka-configs.sh --describe`로 확인해 effective 값으로 설정합니다. 외부 protection window에는 안전하다고 가정하는 기본값이 없습니다. compact/delete 조합이나 topic override를 포함해 값을 알 수 없으면 cleanup을 활성화하지 않습니다. cleanup을 활성화했는데 세 필수 입력 중 하나라도 누락되거나 해석할 수 없으면 애플리케이션 시작을 거부합니다. 모든 기간과 주기는 양수여야 하고 다음 계약을 만족해야 합니다.
...
삭제 대상은 `state=COMMITTED`, `committed_at < cutoff`, 독립 event 또는 `COMPLETED` rebuild event뿐입니다. `RESERVED`, `REDIS_APPLIED`, 미완료·복구 필요 rebuild event는 삭제하지 않습니다. 후보 production SQL은 cleanup 전용 `(state, committed_at)` 인덱스를 `FORCE INDEX`하고 `FOR UPDATE SKIP LOCKED`로 잠근 뒤 삭제 SQL에서 predicate를 다시 확인합니다. 여러 인스턴스가 경합하면 어떤 tick은 0건으로 종료할 수 있으며 다음 주기에 재시도합니다.
