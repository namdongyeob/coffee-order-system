...
- 최종 merge gate의 `PASS`를 증명하는 유일한 machine ground truth는 현재 PR head의 GitHub Actions CI run conclusion입니다. 로컬 테스트 출력과 Agent·Review·QA 로그는 실행 기록과 진단 자료일 뿐 `PASS`의 증명이 아니며 CI run conclusion을 대체하지 않습니다. 이 신뢰 경계는 필요한 Level 3~6 실제 검증을 제거하거나 CI PASS로 대체한다는 뜻이 아니며, 필수 Level 3~6 검증은 기존 위험 기준에 따라 별도로 실행합니다.
- 기본 merge 거버넌스는 사람 도메인 오너의 최종 merge 승인입니다. AI는 Review·QA·CI로 결함을 찾고 사람이 최종 merge를 승인하며, 이 기본값을 최종 팀 프로젝트로 그대로 이전합니다.
- 이 저장소 한정 자율 Issue 큐 실험에서만 모든 필수 gate(독립 Review `APPROVED`, 독립 QA `PASS`, 최신 head CI PASS, merge 안전 조건)를 충족할 때 Main Coordinator의 조건부 auto-merge·close를 예외로 허용합니다. 실험의 고정 큐, 조건부 merge 조건 전체, bootstrap 경계, 안전 정지 규칙은 [자율 큐 runbook](autonomous-queue-runbook.md)에 있으며, 이 실험은 팀 프로젝트로 이전하지 않습니다.
