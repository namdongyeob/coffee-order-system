...
4. DLT 동작이 테스트 또는 검증 가능한 근거로 남아 있는지 확인합니다.
5. 동작 변경에 맞춰 관련 문서가 갱신되었는지 확인합니다.
6. Generic Manager, 숨은 공통 계층, 요구사항과 무관한 리팩터링 같은 과한 추상화와 범위 초과를 확인합니다.
7. 산출물이 프로젝트 근거 없는 일반론, 가짜 검증 주장, 이유 없는 한영 혼합 이름, 동작을 설명하지 못하는 테스트명을 포함하지 않는지 확인합니다.
