...
11-a. Agent register 전에는 실제 존재하는 absolute worktree, 영향도, deadline, `java-required` 또는 `no-java-required` 중 하나를 선언합니다. 시간 입력은 finite 값만 허용합니다. Java·Gradle의 resolved physical path가 non-ASCII이면 `BLOCKED: NON_ASCII_WORKTREE_PATH`, 없는 worktree면 `BLOCKED: WORKTREE_NOT_FOUND`입니다. `lifecycle`은 살아 있는 assignment에만 `WAIT`를 반환하고 `STALLED`·`TIMEOUT`·`BLOCKED`·`COMPLETED`는 slot과 owned path를 소비하지 않으며 heartbeat로 부활하지 않습니다. explicit `block` 명령만 BLOCKED terminal을 기록합니다. diagnostic snapshot은 assignment당 한 번만 state에 소비하고 release 뒤 새 assignment는 새 budget을 받습니다. retry는 Issue 번호와 stable failure key별 `RETRY_ONCE` 한 번으로 제한하고 다음 요청은 `BLOCKED: RETRY_LIMIT`입니다. legacy count에 ledger가 없으면 `BLOCKED: LEGACY_RETRY_LEDGER_REQUIRED`이며, `reset`/`new-run`은 비어 있지 않은 approval reference와 reset history를 state.json에 남깁니다.
11-b. 한국어 Issue/PR metadata는 UTF-8 `body-file`로 전달하고 API read-back에서 한글과 replacement `?` 부재를 확인합니다. `Execution mode:`와 `Execution mode reason:`은 목록 기호 없는 canonical line이어야 합니다.
