...
11. fresh Review·QA·Docs에는 Issue URL, worktree 경로, base/head SHA, Acceptance Criteria, 필수 정본 문서 3~5개 경로, diff 범위, 직전 P0/P1 finding만 전달합니다. 이 allowlist 밖 packet key는 허용하지 않고 문서 경로는 `AGENTS.md`, `docs/ai/*.md`, `docs/testing/*.md`, `.codex/skills/*/SKILL.md`의 canonical repository-relative 경로만 사용합니다. source 본문·전체 conversation·별도 source snapshot을 전달하거나 저장소에 만들지 않으며 역할은 worktree와 GitHub 정본을 직접 읽습니다.
