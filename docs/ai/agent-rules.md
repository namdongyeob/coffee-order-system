...
11-a. Java·Gradle 작업 packet은 ASCII worktree를 사용하고 `worktree_path_action`을 통과해야 합니다. non-ASCII 경로는 `BLOCKED: NON_ASCII_WORKTREE_PATH`입니다. Agent assignment는 `heartbeat`와 `deadline`을 기록하며 `lifecycle_action`은 살아 있는 assignment에 `WAIT`, 정체·만료에는 `STALLED`·`TIMEOUT`을 반환합니다. `retry_decision`은 동일 실패의 자동 retry를 `RETRY_ONCE` 한 번으로 제한하고 다음 실패는 `BLOCKED: RETRY_LIMIT`으로 남깁니다.
