...
11-a. Agent register 전에는 실제 존재하는 absolute worktree, 영향도, deadline, `java-required` 또는 `no-java-required` 중 하나를 선언합니다. Java·Gradle의 resolved physical path가 non-ASCII이면 `BLOCKED: NON_ASCII_WORKTREE_PATH`, 없는 worktree면 `BLOCKED: WORKTREE_NOT_FOUND`입니다. `lifecycle`은 살아 있는 assignment에만 `WAIT`를 반환하고 `STALLED`·`TIMEOUT`·`BLOCKED`·`COMPLETED`는 slot을 소비하지 않으며 heartbeat로 부활하지 않습니다. diagnostic snapshot은 assignment당 한 번만 state에 소비합니다. retry는 Issue 번호와 stable failure key별 `RETRY_ONCE` 한 번으로 제한하고 다음 요청은 `BLOCKED: RETRY_LIMIT`이며, `reset`/`new-run`은 비어 있지 않은 approval reference가 필요합니다.
