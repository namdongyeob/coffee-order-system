...
- 하네스 의무화 이전 Legacy Issue는 원래 merge 당시의 evidence 부재만으로 실패로 판정하지 않습니다. Legacy Issue를 다시 여는 변경은 현재 Issue 번호와 현재 evidence·metrics 형식을 적용하며, 과거 실행 결과를 추정해 backfill하지 않습니다.
