...
- Dev는 변경 범위 focused test를 기본 실행합니다. DB migration, 공통 transaction, event payload, Kafka 공통 consumer 설정, security, build/test infrastructure처럼 영향 범위가 넓은 변경만 push 전 로컬 전체 `./gradlew.bat test --no-daemon` 회귀를 실행합니다. 그 외 변경의 전체 Level 1 회귀는 GitHub Actions `quality-gates`가 최종·단독으로 판정합니다.
