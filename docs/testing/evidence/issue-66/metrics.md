...
- 별도 metadata-only recovery budget은 `eab7171`의 allowlist evidence 형식·EOF 동기화 1회와 `1f30e8b`의 pre-review evidence·verification 동기화 1회로 총 2/2회를 사용했습니다. 하나의 recovery diff commit·push는 수정한 metadata 파일 수와 관계없이 1회이며 preflight, PR comment, CI는 세지 않습니다. 이번 사람 승인 ground-truth reconciliation은 사용량을 늘리지 않습니다. 사용량 정본은 `attempt-log.md`이고 metrics와 PR comment는 이를 참조합니다.
