...
8. 만약 Rebuild가 실패하거나 대조 도중 오류가 발생하면, pending ledger 행은 `COMMITTED`로 업데이트하지 않고 기존 pending 상태(`RESERVED`/`REDIS_APPLIED`)와 증거를 그대로 유지하여 추후 분석 및 재시도가 가능하게 합니다.
9. 복구 락 해제 및 점검창 종료는 **finally 방식(예외 발생 여부와 무관하게 무조건 실행)**으로 안전하게 락을 릴리즈하고 컨슈머 그룹을 재기동합니다.
