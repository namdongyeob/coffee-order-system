...
- **DB Crash Injection Test (트랜잭션 분리 및 Crash 경계 명시)**:
  ...
  - **Crash failpoint 설정**: 테스트 시 crash 주입 지점(failpoint)은 `markRedisApplied(REQUIRES_NEW)` 메서드의 트랜잭션 커밋이 완전히 종료된 직후이자, 이후 최종 완료 처리를 수행하는 `markCommitted` 호출 직전의 경계로 정확하게 명시합니다.
  - **Durability 검증**: 해당 failpoint 시점에 메인 비즈니스 스레드를 강제로 종료(Crash injection)한 뒤, **별도의 독립적인 DB 트랜잭션/커넥션(Separate Transaction/Connection)을 통해 RDBMS를 직접 조회하여 해당 eventId의 ledger 상태가 `REDIS_APPLIED`로 확실히 커밋되어 영구 보존(durability)되었음을 검증**합니다.
